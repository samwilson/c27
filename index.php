...
foreach ($years as $y_num=>$y_name) {
	...
	$page->addBodyContent("<li><a$selected href='/$y_num'>$y_name</a></li>");
}
...
foreach ($all_months as $m_num=>$m_name) {
	...
	if ($m_num == $current_month) {
		...
		$page->addBodyContent("<a href='/$current_year-$m_num' title='$img_count images, $txt_count journal entries.'>$m_name (".($img_count+$txt_count).")</a>");
	}
	$page->addBodyContent("</li>");
}
...
foreach ($db->fetchAll($sql) as $e) {
	...
	$entry['metadata'] = "<a href='/admin/journal.php?id=".$e['id']."&amp;return_to=".urlencode('/'.$current_year.'-'.$current_month."#entry-".$e['id'])."'
		 class='edit-link' title='Edit entry #".$e['id']." (authorisation required).'>[Edit]</a>";
	$entry['class'] = 'journal';
...
foreach ($db->fetchAll($sql) as $image) {
	$entry = array(
		...
		'title' => '',
		'metadata' => "<a href='/admin/images.php?action=edit_image&amp;id=".$image['id']."&amp;return_to=".urlencode('/'.$current_year.'-'.$current_month)."'
			 class='edit-link' title='Edit metadata for image #".$image['id']." (authorisation required).'>[Edit]</a>",
	...
	if ($image['auth_level']==0 || $image['auth_level']<=$auth->getAuthData('auth_level') ) {
		$entry['body'] = "
			<div class=''>
				<div class=''>
					<a href='/images/".$image['id']."/full' title='Date: ".$image['date_and_time'].".  Click for larger view.'>
						<img src='/images/".$image['id']."/view' alt='View this image.' />
					</a>
				</div>
				<div class='caption'>
