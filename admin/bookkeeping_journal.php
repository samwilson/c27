...
function getJournalHeader($curr_year, $curr_month) {
    ...
    $base_uri = $_SERVER['SCRIPT_URI']."?transaction_type=".$_GET['transaction_type'];
    $out = "<div class='bookkeeping-journal-nav'>
        <p>
        <span class='prev'><a href='$base_uri&y=$prev_year&m=$curr_month'>&laquo; $prev_label</a></span>
        <strong class='curr'>$curr_label</strong> <a href='$base_uri&y=$curr_year&m=$curr_month&print'>[print]</a>
...
function getJournalTable($transaction_type, $curr_year, $curr_month) {
    ...
    $sql = "SELECT SUM((1-(private_use_component/100))*amount) AS total FROM bookkeeping
            WHERE transaction_type='$transaction_type' AND MONTH(date)='$curr_month' AND YEAR(date)='$curr_year'";
    ...
    if (!isset($_GET['print'])) {
        ...
        <p class='submit'>
        	...
        	<input type='hidden' name='table_name' value='bookkeeping' />
