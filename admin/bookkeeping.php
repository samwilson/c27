...
function getSummaryTable($transaction_type) {
    ...
    $out = '<div class="span-24 last"><table class="bookkeeping-journal">
    	<caption>Summary of all 
    	<a href="bookkeeping_journal.php?transaction_type='.$transaction_type.'">
    	'.ucfirst($transaction_type).'s
    	</a></caption><tr class="tophead"><th></th>';
    foreach ($cats as $cat) {
    	$out .= "<th>$cat</th>";
    }
    $out .= "<th>Totals</th></tr>";
    $sql = "SELECT MONTHNAME(date) AS month, MONTH(date) AS nmonth, YEAR(date) AS year, SUM(amount) AS total
    	FROM bookkeeping_journal WHERE transaction_type='$transaction_type' GROUP BY MONTHNAME(date), YEAR(date) ORDER BY date DESC";
    $res = mysql_query($sql);
    ...
    $out .= "<tr class='last-row'><td colspan='".(count($cats)+2)."'></td></tr></table></div>";
    return $out;
}
